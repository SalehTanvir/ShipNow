...
export function WarehouseMap() {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 lg:p-6 border border-slate-100 h-full flex flex-col hover:shadow-sm transition-shadow">
        ...
        <div className="flex items-center bg-slate-50 p-1 rounded-lg">
          {floors.map((floor) => (
            <button
              ...
              className={cn(
                "text-[12px] font-medium py-1.5 px-4 rounded-md transition-colors",
      ...
      <div className="bg-[#f8f9fc] rounded-2xl p-4 lg:p-5 flex-1 flex flex-col">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
          {Object.entries(mockMapSections).map(([categoryKey, data]) => {
            ...
            const colSpanClass = categoryKey === 'Apparel' ? 'md:col-span-3' : 'md:col-span-1';

            return (
              <div key={categoryKey} className={cn("bg-white rounded-xl p-4 shadow-sm flex flex-col", colSpanClass)}>
                <h4 className="text-[13px] font-bold text-slate-900 mb-3">{categoryName}</h4>
                <div className="flex flex-wrap gap-2 mb-4">
                  {data.sections.map((section) => (
                    <div 
                      key={section.id} 
                      className={cn(
                        "w-8 h-8 rounded-lg flex items-center justify-center text-[11px] font-bold transition-colors",
                        section.isFull 
                ...
                <div className="text-[11px] text-slate-400 mt-auto">
                  Available Space <span className="text-slate-900 font-bold">{data.availableText}</span>
                </div>
              </div>
            );
          })}
